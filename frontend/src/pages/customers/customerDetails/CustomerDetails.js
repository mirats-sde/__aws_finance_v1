...
const CustomerDetails = () => {
    ...
    axios
      ...
      .then((data) => {
        ...
        axios
          ...
          .then((data) => console.log("customer billing address updated"))
          .catch((err) =>
            console.log("error in customer shipping address" + err)
          );
        axios
  ...
  // *!this is to encrypt and decrypt the client id
  const encryptText = (text) => {
    var b64 = CryptoJS.AES.encrypt(text, "Mirats").toString();
              ...
              <div className={styles.customerLeftContent}>
                ...
                <section className={styles.leftRow}>
                  <div className={styles.border}>
                    <h3>Customer Link</h3>
                    <h3
                      // href={`http://finance.miratsoneservices.com/client/
                      // ${encryptText(String(custData?.customer_id))}`}
                      target="_new"
                      className={styles.customerLink}
                      onClick={(e) =>
                        window.open(
                          ` http://finance.miratsoneservices.com/client/${encryptText(
                            String(custData?.customer_id)
                          )}`
                        )
                      }
                    >
                      http://finance.miratsoneservices.com/client/
                      {encryptText(String(custData?.customer_id))}
                    </h3>
                  </div>
                </section>
